...
async def get_city_by_location(latitude: float, longitude: float) -> Optional[CityInfo]:
    """
    根据经纬度获取城市信息
    """
    # 获取 API Key 和 Host
    ...
    if not api_key or api_key == "your_qweather_api_key_here":
        print("未配置和风天气 API Key")
        return None

    url = f"https://{api_host}/geo/v2/city/lookup"
    params = {
        "location": f"{longitude},{latitude}",  # 注意顺序：经度,纬度
        "number": 1,
        "lang": "zh",
        "key": api_key
    }
...
async def search_city(query: str, limit: int = 10) -> List[CityInfo]:
    ...
    if api_key and api_key != "your_qweather_api_key_here":
        try:
            url = f"https://{api_host}/geo/v2/city/lookup"
            # 和风天气使用 key 参数而不是 Authorization header
            params = {
                "location": query, 
                "number": limit, 
                "lang": "zh",
            ...
            async with httpx.AsyncClient(trust_env=False) as client:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("code") == "200" and data.get("location"):
                        ...
                        print(f"✅ GeoAPI 搜索到 {len(cities)} 个城市")
                        return cities
        except Exception as e:
