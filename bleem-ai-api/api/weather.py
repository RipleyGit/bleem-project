...
@router.get("/city-by-location", response_model=CityInfo)
async def get_city_from_coordinates(
    latitude: float = Query(description="纬度，例如: 39.9042"),
    longitude: float = Query(description="经度，例如: 116.4074")
):
    """
    根据经纬度获取城市信息
    """
    city = await get_city_by_location(latitude, longitude)
    if not city:
        raise HTTPException(
            status_code=404, 
            detail="无法获取城市信息，请检查经纬度是否正确或 API 配置"
        )
    return city
